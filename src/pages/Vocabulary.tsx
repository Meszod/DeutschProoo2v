...
export default function Vocabulary() {
  ...
  async function reviewCard(cardId: string, quality: number) {
    ...
    const { data: updated } = await supabase.from('vocabulary_cards').select('*').eq('id', cardId).single();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { value: cards.length, label: t('vocabulary'), icon: BookOpen, color: 'from-violet-400 to-purple-600', bg: 'bg-violet-50' },
          { value: dueCards.length, label: t('dueToday'), icon: Zap, color: 'from-amber-400 to-orange-500', bg: 'bg-amber-50' },
          { value: cards.length - dueCards.length, label: t('review'), icon: Check, color: 'from-emerald-400 to-teal-500', bg: 'bg-emerald-50' },
        ].map((stat, i) => {
          ...
          return (
            <div key={i} className="card card-hover p-5 text-center group relative overflow-hidden animate-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
              ...
              <div className={`relative w-10 h-10 rounded-xl bg-gradient-to-br ${stat.color} flex items-center justify-center text-white shadow-md mx-auto mb-2 group-hover:scale-110 transition-transform`}>
      ...
      {loading ? (
        ...
        <div className="space-y-4 animate-slide-up">
          ...
          <div onClick={() => setFlipped(!flipped)} className="card card-hover p-12 min-h-[280px] flex flex-col items-center justify-center cursor-pointer relative overflow-hidden group">
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-40 h-40 bg-violet-200/20 rounded-full blur-3xl animate-glow" />
            {!flipped ? (
          ...
          {flipped && (
            <div className="grid grid-cols-2 gap-3 animate-slide-up">
              <button onClick={() => reviewCard(currentCard.id, 2)} className="btn-secondary border-red-200 text-red-600 hover:bg-red-50 hover:scale-105">
                <X className="w-4 h-4" /> {t('didntKnow')}
              </button>
              <button onClick={() => reviewCard(currentCard.id, 5)} className="btn-secondary border-emerald-200 text-emerald-600 hover:bg-emerald-50 hover:scale-105">
