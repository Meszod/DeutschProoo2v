...
import { Shield, Users, FileText, Sparkles, BarChart3, Trash2, Plus, Search, TrendingUp, Activity } from 'lucide-react';

type AdminTab = 'dashboard' | 'users' | 'tasks' | 'generate' | 'analytics';
const TAB_COLORS: Record<AdminTab, string> = { dashboard: '#0ea5e9', users: '#10b981', tasks: '#8b5cf6', generate: '#f59e0b', analytics: '#ec4899' };
    ...
    <Layout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-wrap gap-2 border-b border-slate-200">
          {([['dashboard', 'Dashboard', BarChart3], ['users', 'Foydalanuvchilar', Users], ['tasks', 'Vazifalar', FileText], ['generate', 'Generatsiya', Sparkles], ['analytics', 'Tahlil', TrendingUp]] as const).map(([key, label, Icon]) => (
            <button key={key} onClick={() => setTab(key)} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-all ${tab === key ? 'border-sky-500 text-sky-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ count: users }, { count: tasks }, { count: activeToday }, { count: attemptsToday }] = await Promise.all([
        ...
        supabase.from('profiles').select('*', { count: 'exact', head: true }).gte('last_activity_date', today),
      ...
      {cards.map((c, i) => {
        ...
        return (
          <div key={c.label} className="card card-hover p-5 group relative overflow-hidden animate-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
            ...
            <div className={`relative w-11 h-11 rounded-xl bg-gradient-to-br ${c.gradient} flex items-center justify-center text-white shadow-lg mb-3 group-hover:scale-110 transition-transform`}><Icon className="w-5 h-5" /></div>
            ...
            {filtered.map((u, i) => (
              <div key={u.id} className="flex items-center justify-between px-5 py-3.5 hover:bg-slate-50/50 transition-colors animate-slide-down" style={{ animationDelay: `${i * 0.03}s` }}>
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-sm font-medium text-slate-900">{u.full_name || 'Noma\'lum'}</p><p className="text-xs text-slate-400">{u.target_level || '—'} · {u.current_streak} streak · {u.total_points} pts</p></div>
                </div>
      ...
      <div className="space-y-2">
        {tasks.map((task, i) => (
          <div key={task.id} className="card card-hover p-4 flex items-center justify-between animate-slide-up" style={{ animationDelay: `${i * 0.03}s` }}>
            <div>
              <div className="flex items-center gap-2 mb-1"><span className="badge bg-sky-50 text-sky-600">{task.skill}</span><span className="badge bg-slate-100 text-slate-600">{task.level}</span>{task.teil_number && <span className="text-xs text-slate-400">Teil {task.teil_number}</span>}</div>
              <p className="font-medium text-slate-900 text-sm">{task.title}</p>
            </div>
            <div className="flex gap-1"><button onClick={() => editTask(task)} className="btn-ghost px-2 text-xs">Tahrirlash</button><button onClick={() => deleteTask(task.id)} className="btn-ghost px-2 text-red-500"><Trash2 className="w-4 h-4" /></button></div>
          </div>
        ))}
      </div>
    </div>
...
function AdminGenerate() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-task`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}` }, body: JSON.stringify({ skill, level }) });
  ...
  return (
    <div className="space-y-4">
      <div className="card p-6 space-y-4 animate-slide-up">
        <div className="flex items-center gap-2"><div className="w-9 h-9 rounded-lg bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white shadow-md"><Sparkles className="w-5 h-5" /></div><h3 className="font-semibold text-slate-900">AI orqali vazifa generatsiya qilish</h3></div>
        <p className="text-sm text-slate-500">Claude API (Anthropic) orqali original nemischa matn generatsiya qilinadi.</p>
        ...
        <button onClick={generate} disabled={generating} className="btn-primary">{generating ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Generatsiya...</> : <><Sparkles className="w-4 h-4" /> Generatsiya qilish</>}</button>
      </div>
      {result && <div className="card p-6 animate-slide-up"><h4 className="font-semibold text-slate-900 mb-2">Natija:</h4><pre className="text-xs font-mono text-slate-600 bg-slate-50 p-4 rounded-xl overflow-x-auto whitespace-pre-wrap scrollbar-thin">{result}</pre></div>}
    </div>
  );
}
...
function AdminAnalytics() {
  ...
  attempts.forEach((a) => { if (!skillStats[a.skill]) skillStats[a.skill] = { count: 0, avgPct: 0 }; skillStats[a.skill].count++; if (a.score && a.max_score) skillStats[a.skill].avgPct += (Number(a.score) / Number(a.max_score)) * 100; });
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {Object.entries(skillStats).map(([skill, stats], i) => (
          <div key={skill} className="card card-hover p-5 group animate-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
            <div className="flex items-center gap-2 mb-2"><div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${skillColors[skill]}15` }}><div className="w-3 h-3 rounded-full" style={{ background: skillColors[skill] }} /></div><p className="text-sm font-medium text-slate-500 capitalize">{skill}</p></div>
            <p className="text-2xl font-bold text-slate-900">{stats.count}</p><p className="text-xs text-slate-400 mt-1">O'rtacha: {stats.avgPct}%</p>
          </div>
        ))}
      </div>
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto scrollbar-thin">
          {attempts.slice(0, 50).map((a, i) => (
            <div key={a.id} className="flex items-center justify-between text-sm border-b border-slate-50 py-2 hover:bg-slate-50/50 transition-colors animate-slide-down" style={{ animationDelay: `${i * 0.02}s` }}>
              <div className="flex items-center gap-2"><span className="badge capitalize" style={{ background: `${skillColors[a.skill]}15`, color: skillColors[a.skill] }}>{a.skill}</span><span className="text-slate-500">{a.level || '—'}</span></div>
              <div className="flex items-center gap-2"><span className="text-slate-900 font-medium">{a.score}/{a.max_score}</span><span className="text-xs text-slate-400">{new Date(a.created_at).toLocaleDateString()}</span></div>
            </div>
          ))}
        </div>
      </div>
