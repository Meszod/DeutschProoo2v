      ...
      <div className="card p-6 space-y-5 animate-slide-up">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {LEVELS.map((lvl) => (
              <button key={lvl} onClick={() => setTargetLevel(lvl)} className={`level-pill ${targetLevel === lvl ? 'text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`} style={targetLevel === lvl ? { background: 'linear-gradient(135deg, #0ea5e9, #0284c7)' } : {}}>
                {lvl}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            {(['uz', 'ru', 'en'] as const).map((l) => (
              <button key={l} onClick={() => setLang(l)} className={`level-pill ${lang === l ? 'text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`} style={lang === l ? { background: 'linear-gradient(135deg, #0ea5e9, #0284c7)' } : {}}>
                {l.toUpperCase()}
              </button>
            ))}
          </div>
