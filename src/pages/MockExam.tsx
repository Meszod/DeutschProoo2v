  ...
  async function finishExam() {
    ...
    if (user && level) {
      await supabase.from('mock_exams').insert({
        user_id: user.id, level, exam_type: profile?.target_exam_type || 'Goethe',
        lesen_score: scores.lesen || 0, hoeren_score: scores.hoeren || 0,
        schreiben_score: scores.schreiben || 0, sprechen_score: scores.sprechen || 0,
        total_score: total, passed,
      });
              ...
              {history.map((exam, i) => (
                <div key={exam.id} className="card card-hover p-4 flex items-center justify-between animate-slide-up" style={{ animationDelay: `${i * 0.03}s` }}>
                  <div>
                    ...
                    <span className="text-sm text-slate-400 ml-2">{new Date(exam.created_at).toLocaleDateString()}</span>
                  </div>
    ...
    return (
      <div className="space-y-5 animate-fade-in">
        <div className="flex items-center justify-between flex-wrap gap-3 animate-slide-down">
          <button onClick={() => setPhase('intro')} className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 group transition-colors">
            <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> {t('back')}
          </button>
          <div className="flex items-center gap-2">
            <span className="badge" style={{ background: `${ACCENT}15`, color: ACCENT }}>{level}</span>
            ...
            {scores[phase] !== undefined && (
              <div className="card p-6 text-center animate-scale-in">
                ...
                <button onClick={nextPart} className="btn-primary mt-4">
                  {phase === 'sprechen' ? t('finish') : t('nextPart')} <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            )}
          </>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-6 animate-slide-up">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {LEVELS.map((lvl) => (
            <button key={lvl} onClick={() => setLevel(lvl)} className={`level-pill ${level === lvl ? 'text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`} style={level === lvl ? { background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT}dd)` } : {}}>
              {lvl}
            </button>
          ))}
        </div>
    ...
    <div className="space-y-4 animate-fade-in">
            ...
            <ul className="space-y-1.5">
              {task.content.leitpunkte.map((lp, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-slate-600 animate-slide-down" style={{ animationDelay: `${i * 0.05}s` }}>
                  <span className="font-bold text-indigo-500">•</span>{lp}
                </li>
              ))}
            </ul>
          </div>
      ...
      {mode === 'schreiben' ? (
        ...
        <div className="card p-6 text-center text-slate-500">
          ...
          <textarea className="input mt-4 min-h-[100px]" placeholder="Sizning javobingiz matnini shu yerga kiriting (mock rejim)..." value={text} onChange={(e) => setText(e.target.value)} disabled={score !== null} />
        </div>
      )}
      {score !== null ? (
        <div className="card p-6 text-center animate-scale-in">
