                  ...
                  return (
                    <div key={entry.user_id} className={`flex flex-col items-center justify-end ${scales[displayIdx]} animate-bounce-in`} style={{ animationDelay: `${displayIdx * 0.1}s` }}>
                      <div className="relative">
                        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 text-slate-600 flex items-center justify-center text-lg font-bold shadow-md">{(entry.full_name || '?').charAt(0).toUpperCase()}</div>
                        <div className="absolute -top-2 -right-2 w-7 h-7 rounded-full flex items-center justify-center text-white shadow-md" style={{ background: medalColors[displayIdx] }}><Icon className="w-4 h-4" /></div>
                      </div>
                      <p className="text-xs font-medium text-slate-700 mt-2 truncate max-w-full px-1">{entry.full_name || '—'}{isMe && ' (siz)'}</p>
                      <div className={`${heights[displayIdx]} w-full rounded-t-2xl mt-2 flex items-center justify-center text-white font-bold text-xl shadow-md`} style={{ background: `linear-gradient(135deg, ${medalColors[displayIdx]}, ${medalColors[displayIdx]}dd)` }}>{entry.total_points}</div>
                    </div>
                  );
                })}
              </div>
                ...
                {entries.map((entry, idx) => {
                  ...
                  return (
                    <div key={entry.user_id} className={`grid grid-cols-12 gap-2 px-5 py-3.5 items-center transition-colors hover:bg-slate-50/50 ${isMe ? 'bg-amber-50/50' : idx < 3 ? 'bg-amber-50/20' : ''}`}>
                      <div className="col-span-1 flex items-center">{Icon ? <Icon className="w-5 h-5" style={{ color: medalColors[idx] }} /> : <span className="text-sm font-medium text-slate-400">{idx + 1}</span>}</div>
                      ...
                      <div className="col-span-3 text-right flex items-center justify-end gap-1.5"><Star className="w-3.5 h-3.5 text-amber-400" /><span className="text-sm font-bold text-slate-900">{entry.total_points}</span></div>
                    </div>
                  );
                })}
              </div>
